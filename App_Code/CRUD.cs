...
/// </summary>
/// 
namespace ibtisam4439.App_Code
{
   public class CRUD
    {
        ...
        public static string conStr = WebConfigurationManager.ConnectionStrings["ibtisam4439ConStr"].ConnectionString;
        // I create a con object once and then use it in methods
        public SqlConnection con = new SqlConnection(WebConfigurationManager.ConnectionStrings["ibtisam4439ConStr"].ConnectionString);
        public CRUD()
        {
        ...
        public DataTable GetDataTable(string query)
        {
            ...
            using (SqlConnection connection = new SqlConnection(conStr))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    // Create a SqlDataAdapter to fill the DataTable with the result
            ...
            // Return the DataTable with the result
            return dataTable;
        }

        public T GetScalar<T>(string query, Dictionary<string, object> parameters)
        {
            using (SqlConnection connection = new SqlConnection(conStr))
            {
                connection.Open();
        ...
        public DataRow GetDataRow(string query, Dictionary<string, object> parameters)
        {
        ...
        public bool IsUserInRole(string email, string roleName)
        {
            using (SqlConnection conn = new SqlConnection(conStr))
            {
                ...
                string query = @"
                                SELECT COUNT(*)
                                FROM visitors v
                                JOIN visitorRoles vr ON v.visitorId = vr.visitorId
                                JOIN Roles r ON vr.RoleId = r.RoleID
                                WHERE v.email = @email AND r.RoleName = @roleName";
